...
def generate_content(prompt, model_version, retries=5, backoff_factor=5):
    """Generate content using Claude API with retry mechanism."""
    for attempt in range(retries):
        try:
            message = client.messages.create(
                model=model_version,  # Use the selected model version
                max_tokens=2500,
                temperature=0.4,
                messages=[
            ...
            if e.status_code == 529:
                console.insert(tk.END, f"Rate limit exceeded, retrying in {backoff_factor ** attempt} seconds...\n")
                time.sleep(backoff_factor ** attempt)
            else:
                console.insert(tk.END, f"Another non-200-range status code was received: {e.status_code}\n")
                console.insert(tk.END, str(e.response) + "\n")
                break
...
def generate_book():
    ...
    model_version = model_combobox.get()  # Get the selected model version
    ...
    for chapter_num in range(1, num_chapters + 1):
        ...
        while not chapter_generated:
            try:
                ...
                chapter_title = generate_content(chapter_title_prompt, model_version).strip()
                ...
                chapter_content = generate_content(chapter_content_prompt, model_version).replace("To be continued...", "").strip()
...
tk.Label(root, text="Model Version").grid(row=4, column=0, sticky='w')  # Move the model version label to row 4
...
model_combobox = ttk.Combobox(root, width=48, values=["claude-3-opus-20240229", "claude-3-sonnet-20240229", "claude-2.1", "claude-instant-1.2"])
...
model_combobox.grid(row=4, column=1, padx=10, pady=10)  # Move the model selection box to row 4

chapters_combobox.current(0)  # Set the default value to 1
model_combobox.current(0)  # Set the default value to "claude-3-opus-20240229"

tk.Button(root, text="Generate Book", command=generate_book).grid(row=5, column=0, columnspan=2, pady=10)  # Move the generate button to row 5

progress_bar = ttk.Progressbar(root, orient='horizontal', mode='determinate', length=300)
progress_bar.grid(row=6, column=0, columnspan=2, pady=10)  # Move the progress bar to row 6

console_label = tk.Label(root, text="Output Console")
console_label.grid(row=7, column=0, sticky='w', padx=10, pady=5)  # Move the console label to row 7

console = scrolledtext.ScrolledText(root, width=80, height=10)
console.grid(row=8, column=0, columnspan=2, padx=10, pady=5)  # Move the console to row 8
